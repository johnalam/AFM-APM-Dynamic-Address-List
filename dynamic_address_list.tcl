...
when ACCESS_POLICY_COMPLETED {
    ...
    set group [ACCESS::session data get session.cust.user.group]
    set address_list "apm_accesslist_$group"
    if {[info exists ip_address]} {
	    # create a random secret
	    if {[catch {set res [ILX::call $ilx_handle "addAddress" $ip_address $address_list]} result]} {
...
when ACCESS_SESSION_CLOSED {
    ...
    if {[info exists ip_address]} {
	    # create a random secret
	    if {[catch {set res [ILX::call $ilx_handle "deleteAddress" $ip_address $address_list]} result]} {
